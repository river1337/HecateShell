...
- Every 1 second, reads both `config.json` and `theme.json` via ColorLoader
- Compares content with last read
- If changed, parses JSON and updates properties
...
**Two config files are hot-reloaded:**
1. `theme.json` - Colors only (generated by matugen)
2. `config.json` - Everything else (bar height, fonts, spacing, visualizer, icons, etc.)

Color transitions are animated (300ms), size/spacing transitions are animated (200ms).
...
When adding new configurable properties to the shell:

1. **Add property to Config.qml** with a sensible default value:
   ```qml
   property int myNewOption: 10
   ```

2. **Add to config.json** under the appropriate section (or create new section):
   ```json
   {
       "mySection": {
           "myNewOption": 10
...
3. **Add parsing in `parseConfig()` function** in Config.qml:
   ```qml
   if (cfg.mySection) {
       if (cfg.mySection.myNewOption !== undefined) config.myNewOption = cfg.mySection.myNewOption
   }
   ```

4. **Add Behavior animation** if the property should animate (optional):
   ```qml
   Behavior on myNewOption { NumberAnimation { duration: 200; easing.type: Easing.InOutQuad } }
   ```

5. **Use in components** via `Shell.Config.myNewOption`

**Important:** Always use `!== undefined` checks in parseConfig() so missing keys don't overwrite defaults with undefined. Derived properties (like `workspaceItemSize`) are calculated from other config values and don't need JSON entries.
