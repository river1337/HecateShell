...
matugen json ~/.config/HecateShell/theme.json -m dark --json hex --continue-on-error

# From wallpaper (outputs to theme.json)
matugen image ~/wallpaper.png -m dark --json hex -o ~/.config/HecateShell/theme.json --continue-on-error

# From color
matugen color '#D5C4AA' -m dark --json hex -o ~/.config/HecateShell/theme.json --continue-on-error
```
...
- Every 1 second, reads `$HOME/.config/HecateShell/theme.json`
...
2. Matugen generates `theme.json` from wallpaper/color/existing palette
3. Config.qml hot-reloads `theme.json` every 1 second (path dynamically resolved)
4. JSON is parsed and Material Design 3 colors mapped to shell properties
...
- No longer uses matugen templates - reads `theme.json` directly for simplicity
